...
TASK_ID = "TASK-194"
CAMPAIGN_NAME = "WDI Barro-Lee Education Attainment Domain Completion Campaign"
CAMPAIGN_MODE = "Repository Expansion with Domain Completion Prioritization"
SOURCE_NAME = "World Bank World Development Indicators / Barro-Lee educational attainment indicators"
COUNTRY_CATALOG_FIXTURE = PROJECT_ROOT / "data/raw/wdi_operational_phase1/wdi-phase1-all-countries-3i-2000-2023.json"
RAW_PATH = PROJECT_ROOT / "data/raw/task194_wdi_education_attainment_closure/task-194-wdi-education-attainment-72i-1990-2024.json"
NORMALIZED_PATH = PROJECT_ROOT / "data/processed/task194_wdi_education_attainment_closure/task-194-wdi-education-attainment-normalized.json"
...
def _get_json(url: str, timeout_seconds: int) -> Any:
    req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-194 WDI education attainment closure expansion"})
    with urllib.request.urlopen(req, timeout=timeout_seconds) as response:
        return json.loads(response.read().decode("utf-8"))


def fetch_raw(timeout_seconds: int = 30) -> dict[str, Any]:
    ...
    def fetch_one(indicator: str) -> dict[str, Any]:
        ...
        return {"indicator_code": indicator, "url": data_url, "metadata_url": meta_url, "response": payload, "metadata_response": metadata_payload}
    ...
    return {
        "scope": {
            ...
            "domain": "Education and human-capital attainment",
            "analytical_capability": "Global educational attainment distribution and schooling-stock monitoring",
            "confidence_cell": "WDI public API v2 annual scalar country-indicator observations for Barro-Lee education attainment series",
    ...
    for req in raw["requests"]:
        ...
        for item in obs:
            ...
            rows.append({
                ...
                "repository_section": "labor_closure_depth_access_stability",
                "operational_capability": "Global educational attainment distribution and schooling-stock monitoring",
...
def reports(raw: dict[str, Any], load_counts: dict[str, int] | None = None, db_before: dict[str, int] | None = None, db_after: dict[str, int] | None = None) -> dict[str, Any]:
    ...
    return {
        "selection": {
            "task": TASK_ID,
            "status": "complete",
            "selected_domain": "Education and human-capital attainment",
            "selected_capability": "Global educational attainment distribution and schooling-stock monitoring",
            "capability_gap": "MacroForge had broad WDI education attainment and human-capital coverage, but the labor-closure domain lacked a broad Barro-Lee country-year/age/sex educational attainment distribution panel.",
            "confidence_cell": "WDI public API v2 annual scalar country-indicator observations for Barro-Lee education attainment series",
            "candidate_count": len(CANDIDATE_INDICATORS),
            "included_indicator_count": norm["indicator_count"],
            "excluded_indicator_count": len(norm["excluded_indicators"]),
            "selection_rationale": "Selected because the education/human-capital domain is already substantially developed and this large GFDD-compatible campaign moves it toward operational completeness without changing architecture.",
            "provider_selection_note": "Provider selection followed capability selection; WDI/Barro-Lee was used because it supplied the selected labor-closure annual scalar confidence cell.",
        },
        "expansion": {
            "task": TASK_ID,
            "status": "complete",
        ...
        "domain_progress": {
            ...
            "status": "complete",
            "domain": "Education and human-capital attainment",
            "capability": "Global educational attainment distribution and schooling-stock monitoring",
            "capability_before": "Operationally useful for broad WDI macro-financial/external-vulnerability signals, but incomplete for labor-closure structure: limited coverage of inclusion/access, bank efficiency, stability, ownership/concentration, securities-market depth, and nonbank/intermediary depth.",
            "capability_after": "Materially advanced: repository now supports broad annual country-panel monitoring of GFDD financial access/inclusion, institutional depth, market depth, bank efficiency, bank stability, ownership/concentration, securities/debt-market indicators, and remittance/nonresident-bank exposure signals inside the WDI/Barro-Lee annual-scalar cell.",
            "domain_completion_assessment": "Approaching operational completeness inside the WDI/Barro-Lee annual-scalar labor-closure confidence cell. Still not complete for detailed BOP/IIP instrument/counterparty structure, quarterly banking systems, supervisory bank-level data, full securities issuance microstructure, or non-WDI cross-provider validation.",
            "remaining_first_order_gaps": [
                "quarterly/monthly labor-closure and banking indicators",
...
def write_all(raw: dict[str, Any], load_counts: dict[str, int] | None = None, db_before: dict[str, int] | None = None, db_after: dict[str, int] | None = None) -> None:
    ...
    names = {
        "selection": "task-193-campaign-selection-report.json",
        "expansion": "task-193-repository-expansion-report.json",
        "postgres_growth": "task-193-postgresql-growth-report.json",
        "domain_progress": "task-193-domain-progress-report.json",
        "provider_evidence": "task-193-provider-evidence-classification-report.json",
        "architecture_observation": "task-193-architecture-to-reality-observation-report.json",
    }
