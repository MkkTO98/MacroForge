...
EXPECTED_FINGERPRINT = "4df65129b38d6518cd8d0d8bc7e77f44c98f9abea352b697145124ce9e3033bb"
...
def test_wdi_macro_indicators_normalizes_operational_coverage() -> None:
    normalized = normalize_wdi_macro_indicators_fixture(_payload())
...
def test_wdi_macro_indicators_builds_deterministic_observed_package() -> None:
    left = build_wdi_macro_indicators_observed_package(_payload())
    right = build_wdi_macro_indicators_observed_package(_payload())
...
def test_wdi_macro_indicators_writes_normalized_and_refresh_manifest(tmp_path: Path) -> None:
    ...
    normalized = write_wdi_macro_indicators_normalized_artifact(_payload(), normalized_path)
    manifest = write_wdi_macro_indicators_refresh_manifest(_payload(), manifest_path, normalized_path=normalized_path)
...
def test_wdi_macro_indicators_loads_to_isolated_postgres_when_available(tmp_path: Path) -> None:
    ...
    normalized_path = tmp_path / "normalized.json"
    write_wdi_macro_indicators_normalized_artifact(_payload(), normalized_path)
    try:
        try:
