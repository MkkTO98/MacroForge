...
SCRIPT_PATH = PROJECT_ROOT / "tools/task205_wdi_agriculture_rural_development_chunked_expansion.py"


def _load_task205_module():
    spec = importlib.util.spec_from_file_location("task205_wdi_agriculture_rural_development_chunked_expansion", SCRIPT_PATH)
...
def test_task205_classify_does_not_convert_acquisition_error_placeholder_to_zero_observation() -> None:
    task205 = _load_task205_module()
    raw = {
        "scope": {
            "countries": ["USA"],
...
def test_task205_unresolved_acquisition_errors_block_completion_claims() -> None:
    task205 = _load_task205_module()
    manifest = {
        "candidate_count": 2,
    ...
    completion = task205.completion_semantics(manifest)
...
def test_task205_completion_claims_allowed_when_no_acquisition_errors_remain() -> None:
    task205 = _load_task205_module()
    manifest = {
        "candidate_count": 2,
        "included_indicator_count": 1,
