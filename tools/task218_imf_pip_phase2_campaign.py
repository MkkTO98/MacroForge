...
TASK_ID = "TASK-218"
SLUG = "task218_imf_pip_phase2_campaign"
SOURCE_CODE = "IMF_SDMX_PIP_API_V1"
SOURCE_NAME = "International Monetary Fund SDMX PIP API"
SOURCE_HOME_URL = "https://data.imf.org/"
PROVIDER_DATASET_CODE = "IMF:PIP"
DATAFLOW_CODE = "PIP"
DATAFLOW_VERSION = "5.0.0"
DSD_VERSION_EXPECTED = "5.0.0"
RUN_KEY = "task-218-imf-pip-portfolio-counterpart-phase2"
PIPELINE_NAME = "imf_pip_portfolio_counterpart_phase2_campaign"
FREQUENCY = "A"
UNIT_CODE = "USD"
...
SERIES_KEY_DIMENSIONS = ["COUNTRY", "ACCOUNTING_ENTRY", "INDICATOR", "SECTOR", "COUNTERPART_SECTOR", "COUNTERPART_COUNTRY", "FREQUENCY"]
TERRITORY_DIMENSION = "COUNTRY"
COUNTERPART_DIMENSION = "COUNTERPART_COUNTRY"
METADATA_URL = "https://api.imf.org/external/sdmx/2.1/dataflow/all/PIP/latest?references=all"
RAW_DIR = PROJECT_ROOT / "data/raw" / SLUG
PROCESSED_DIR = PROJECT_ROOT / "data/processed" / SLUG
REPORT_DIR = PROJECT_ROOT / "artifacts/reports"
PRED_PATH = REPORT_DIR / "task-218-imf-pip-frozen-pre-execution-prediction.json"
PROVIDER_REPORT = REPORT_DIR / "task-218-imf-pip-provider-structure-and-evidence-report.json"
NORM_PATH = PROCESSED_DIR / "active" / "task-218-imf-pip-normalized.json"
MANIFEST_PATH = PROCESSED_DIR / "active" / "task-218-imf-pip-manifest.json"
LOAD_REPORT = REPORT_DIR / "task-218-imf-pip-postgresql-load-report.json"
IDEMPOTENCE_REPORT = REPORT_DIR / "task-218-imf-pip-postgresql-idempotence-report.json"
EVAL_REPORT = REPORT_DIR / "task-218-imf-pip-prediction-evaluation.json"
EXTRACTION_REPORT = REPORT_DIR / "task-218-imf-pip-extraction-decision.json"
CHECKSUMS = REPORT_DIR / "task-218-imf-pip-artifact-checksums.txt"
SQL_PATH = REPORT_DIR / "task-218-imf-pip-load.sql"
...
def canonical_indicator_code(indicator: str, counterpart: str) -> str:
    return f"IMF:PIP:{ACCOUNTING_ENTRY}:{indicator}:SECTOR_{SECTOR}:COUNTERPART_SECTOR_{COUNTERPART_SECTOR}:COUNTERPART_{counterpart}:{UNIT_CODE}:SCALE_{UNIT_SCALE}:{FREQUENCY}"
...
def source_url(countries: list[str], counterparts: list[str]) -> str:
    ...
    return f"https://api.imf.org/external/sdmx/2.1/data/{DATAFLOW_CODE}/{c}.{ACCOUNTING_ENTRY}.{inds}.{SECTOR}.{COUNTERPART_SECTOR}.{cp}.{FREQUENCY}?startPeriod={START_PERIOD}&endPeriod={END_PERIOD}"


def fetch_url(url: str, *, timeout: int = 240) -> tuple[bytes, dict[str, Any]]:
    req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-218 IMF PIP Phase2"})
...
def fetch_metadata_active() -> dict[str, Any]:
    ...
    (attempt_dir / "task-218-imf-pip-metadata.xml").write_bytes(data)
    write_json(attempt_dir / "task-218-imf-pip-metadata.json", meta)
    active = RAW_DIR / "active"
    tmp = RAW_DIR / ".active-metadata.tmp"
    if tmp.exists(): shutil.rmtree(tmp)
    tmp.mkdir(parents=True)
    (tmp / "task-218-imf-pip-metadata.xml").write_bytes(data)
    write_json(tmp / "task-218-imf-pip-metadata.json", meta)
...
def parse_metadata(metadata_path: Path | None = None) -> dict[str, Any]:
    path = metadata_path or RAW_DIR / "active" / "task-218-imf-pip-metadata.xml"
    root = ET.fromstring(path.read_bytes())
    codelists: dict[str, dict[str, str]] = {}
    keep = {"CL_COUNTRY", "CL_PIP_COUNTRY", "CL_ACCOUNTING_ENTRY", "CL_PIP_INDICATOR", "CL_SECTOR", "CL_UNIT", "CL_FREQ"}
    for cl in root.iter():
...
def selected_economies(meta: dict[str, Any]) -> dict[str, str]:
    provider = meta["codelists"].get("CL_COUNTRY") or meta["codelists"].get("CL_PIP_COUNTRY") or {}
...
def write_provider_report_and_prediction() -> dict[str, Any]:
    if not (RAW_DIR / "active" / "task-218-imf-pip-metadata.xml").exists():
    ...
    for reporter in SELECTED_ECONOMIES:
        for counterpart in SELECTED_ECONOMIES:
            for ind, label in SELECTED_SERIES.items():
                candidate_series.append({"series_key": f"{reporter}.{ACCOUNTING_ENTRY}.{ind}.{SECTOR}.{COUNTERPART_SECTOR}.{counterpart}.{FREQUENCY}", "reporter_country": reporter, "counterpart_country": counterpart, "accounting_entry": ACCOUNTING_ENTRY, "indicator": ind, "indicator_label": label, "sector": SECTOR, "counterpart_sector": COUNTERPART_SECTOR, "frequency": FREQUENCY})
    expected_cells = len(candidate_series) * len(p)
    provider_report = {"task": TASK_ID, "source_api": "IMF external SDMX 2.1 API /external/sdmx/2.1/data/PIP and dataflow/all/PIP/latest?references=all", "provider_dataset_code": PROVIDER_DATASET_CODE, "dataflow": meta["dataflow"], "data_structure": meta["data_structure"], "series_key_dimensions": SERIES_KEY_DIMENSIONS, "territory_dimension": TERRITORY_DIMENSION, "counterpart_dimension": COUNTERPART_DIMENSION, "selected_economies": [{"provider_code": k, "canonical_code": k, "label": countries[k]} for k in SELECTED_ECONOMIES], "selected_indicators": [{"indicator": k, "label": v} for k, v in SELECTED_SERIES.items()], "sector_selection": {"sector": SECTOR, "counterpart_sector": COUNTERPART_SECTOR}, "temporal_coverage_selected_window": {"start": START_PERIOD, "end": END_PERIOD, "periods": len(p)}, "candidate_series_count": len(candidate_series), "candidate_cell_count": expected_cells, "selected_capability": "Annual IMF PIP/CPIS portfolio investment positions by reporting economy, counterpart economy, and instrument family for a 24-economy investment-relevant matrix."}
    write_json(PROVIDER_REPORT, provider_report)
    prediction = {"task": TASK_ID, "frozen_before_value_acquisition": True, "selected_source": SOURCE_CODE, "provider_dataset_code": PROVIDER_DATASET_CODE, "release_identity_rule": "derive PIP as-of key from provider dataset UPDATE_DATE/PUBLICATION_DATE/Prepared evidence", "run_key": RUN_KEY, "selected_capability": provider_report["selected_capability"], "confidence_cell": "IMF SDMX annual scalar relationship-position values with reporter territory as canonical territory and counterpart economy preserved in source-scoped indicator identity/attributes", "selected_economies": provider_report["selected_economies"], "selected_indicators": provider_report["selected_indicators"], "exact_provider_advertised_series_count": len(candidate_series), "exact_provider_advertised_series": candidate_series, "expected_candidate_cells": expected_cells, "expected_provider_valued_facts": int(expected_cells * 0.70), "expected_explicit_missing_facts": int(expected_cells * 0.15), "expected_whole_series_absence": expected_cells - int(expected_cells * 0.70) - int(expected_cells * 0.15), "expected_units": {"unit": UNIT_CODE, "scale": UNIT_SCALE, "unit_label": UNIT_LABEL}, "expected_transport_behavior": "four reporter chunks against 24 counterpart economies; metadata large but proven accessible", "expected_implementation_friction": "moderate-high: PIP adds counterpart-country relationship semantics but should fit scalar storage by retaining counterpart in indicator attributes", "expected_postgresql_growth": {"facts": "observed plus explicit-missing rows only", "indicators": len(SELECTED_ECONOMIES) * len(SELECTED_SERIES)}, "structural_assumptions_pressure_tested": ["relationship/counterpart economy can be preserved without schema redesign for bounded PIP matrix", "provider as-of identity remains dataset/release metadata", "whole-series absence remains distinct from explicit missing years", "same-run idempotence holds after deleting/reloading run-scoped facts"]}
    write_json(PRED_PATH, prediction)
    return prediction


def write_extraction_decision() -> dict[str, Any]:
    report = {"task": TASK_ID, "decision": "No shared IMF/PIP or relationship-position substrate extracted", "evidence_compared": ["TASK-216 IMF BOP current-account flows", "TASK-217 IMF IIP aggregate stocks", "TASK-218 IMF PIP counterpart portfolio positions"], "repeated_responsibilities": ["IMF SDMX metadata parsing", "provider-derived as-of identity", "attempt-specific acquisition and active promotion", "explicit-missing and whole-series absence reconciliation"], "rejected_extraction_reason": "PIP introduces counterpart-country relationship semantics distinct from BOP/IIP aggregate scopes. Repetition is visible, but extracting now risks a generic SDMX or external-sector framework before relationship identity has repeated stable contracts.", "explicitly_not_created": ["universal SDMX machinery", "generic campaign engine", "counterparty ontology", "schema redesign"]}
...
def fetch_values() -> dict[str, Any]:
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            meta.update({"task": TASK_ID, "status": "value_chunk_acquired", "attempt_id": attempt_id, "chunk_index": idx, "chunk_count": len(chunks), "reporters": chunk, "counterparts": counterparts, "request_parameters": {"dataflow": DATAFLOW_CODE, "reporters": chunk, "accounting_entry": ACCOUNTING_ENTRY, "indicators": list(INDICATORS), "sector": SECTOR, "counterpart_sector": COUNTERPART_SECTOR, "counterparts": counterparts, "frequency": FREQUENCY, "startPeriod": START_PERIOD, "endPeriod": END_PERIOD}})
            raw_name=f"task-218-imf-pip-values-chunk-{idx:02d}.xml"; meta_name=f"task-218-imf-pip-values-chunk-{idx:02d}.json"
            ...
            write_json(attempt_dir/f"task-218-imf-pip-values-chunk-{idx:02d}-error.json", err); errors.append(err)
    if errors:
    ...
    write_json(tmp/"task-218-imf-pip-raw-values-manifest.json", manifest)
    active.mkdir(parents=True, exist_ok=True)
    for p in tmp.iterdir(): shutil.move(str(p), str(active/p.name))
...
def provider_as_of_from_responses(headers: list[dict[str, Any]]) -> tuple[str, str]:
    ...
    if not date: raise ValueError("cannot derive IMF PIP as-of identity from provider metadata")
    token=date.replace(":", "").replace("-", "").replace(".", "").replace("Z", "z").replace("T", "t")[:32].lower()
    as_of_date=date[:10]
    return f"imf-pip-asof-{token}", as_of_date


def normalize() -> dict[str, Any]:
    pred=json.loads(PRED_PATH.read_text(encoding="utf-8")); active=RAW_DIR/"active"
    value_manifest=json.loads((active/"task-218-imf-pip-raw-values-manifest.json").read_text(encoding="utf-8"))
    meta=parse_metadata(active/"task-218-imf-pip-metadata.xml")
    ...
    for item in value_manifest["responses"]:
        ...
        for series in root.iter():
            ...
            c=attrs.get("COUNTRY"); acct=attrs.get("ACCOUNTING_ENTRY"); ind=attrs.get("INDICATOR"); sec=attrs.get("SECTOR"); csec=attrs.get("COUNTERPART_SECTOR"); cp=attrs.get("COUNTERPART_COUNTRY"); freq=attrs.get("FREQUENCY")
            if c not in selected_countries or cp not in selected_countries or acct!=ACCOUNTING_ENTRY or ind not in selected_ind or sec!=SECTOR or csec!=COUNTERPART_SECTOR or freq!=FREQUENCY:
        ...
        for cp in SELECTED_ECONOMIES:
            for ind,label in SELECTED_SERIES.items():
                ...
                for per in p:
                    ...
                    attrs={"task":TASK_ID,"repository_section":"External Sector","provider_dataset_code":PROVIDER_DATASET_CODE,"dataflow_code":DATAFLOW_CODE,"dataflow_version":meta["dataflow"].get("version"),"data_structure_version":meta["data_structure"].get("version"),"pip_accounting_entry":ACCOUNTING_ENTRY,"pip_indicator":ind,"pip_indicator_label":label,"reporter_country":c,"counterpart_country":cp,"sector":SECTOR,"counterpart_sector":COUNTERPART_SECTOR,"unit":UNIT_CODE,"scale":sattrs.get("SCALE",UNIT_SCALE),"frequency":FREQUENCY,"derivation_type":None if obs is None else obs.get("DERIVATION_TYPE"),"access_sharing_level":None if obs is None else obs.get("ACCESS_SHARING_LEVEL"),"security_classification":None if obs is None else obs.get("SECURITY_CLASSIFICATION"),"value_status":"provider_pip_value_status_unspecified"}
...
def build_load_sql(norm: dict[str, Any], run_key: str = RUN_KEY) -> str:
    ...
    raw_paths=[rel(p) for p in sorted((RAW_DIR/"active").glob("task-218-imf-pip-values-chunk-*.xml"))]
...
INSERT INTO meta.source (source_code, source_name, source_home_url, license_note) VALUES ({sql_literal(SOURCE_CODE)}, {sql_literal(SOURCE_NAME)}, {sql_literal(SOURCE_HOME_URL)}, 'IMF public SDMX PIP API') ON CONFLICT (source_code) DO UPDATE SET source_name=EXCLUDED.source_name, source_home_url=EXCLUDED.source_home_url;
...
INSERT INTO staging.task218_imf_pip_portfolio_counterpart_observation (pipeline_run_id, source_id, dataset_release_id, territory_code, territory_label, indicator_code, indicator_name, provider_period_code, period_year, value, unit_code, unit_label, decimal_precision, observation_status, attribute_hash, attributes, source_payload) SELECT _run_row.pipeline_run_id, _source_row.source_id, _release_row.dataset_release_id, r.* FROM _task218_rows r CROSS JOIN _run_row CROSS JOIN _source_row CROSS JOIN _release_row ON CONFLICT (pipeline_run_id, territory_code, indicator_code, provider_period_code, unit_code, attribute_hash) DO UPDATE SET value=EXCLUDED.value, observation_status=EXCLUDED.observation_status, attributes=EXCLUDED.attributes, source_payload=EXCLUDED.source_payload;
INSERT INTO curated.dim_indicator (source_id, source_indicator_code, indicator_name, description, topic) SELECT DISTINCT s.source_id, indicator_code, indicator_name, 'IMF PIP source-scoped scalar indicator preserving portfolio instrument and counterpart economy in indicator identity/attributes', 'External Sector / Portfolio Investment Positions' FROM _task218_rows CROSS JOIN _source_row s ON CONFLICT (source_id, source_indicator_code) DO UPDATE SET indicator_name=EXCLUDED.indicator_name, description=EXCLUDED.description, topic=EXCLUDED.topic;
INSERT INTO curated.dim_unit (unit_code, unit_name, unit_description) SELECT DISTINCT unit_code, unit_label, 'IMF PIP USD unit with provider scale preserved' FROM _task218_rows ON CONFLICT (unit_code) DO UPDATE SET unit_name=EXCLUDED.unit_name, unit_description=EXCLUDED.unit_description;
INSERT INTO curated.dim_period (frequency, period_year, period_start_date, period_end_date, period_label) SELECT DISTINCT 'A', period_year, make_date(period_year,1,1), make_date(period_year,12,31), provider_period_code FROM _task218_rows ON CONFLICT (frequency, period_start_date, period_end_date) DO UPDATE SET period_year=EXCLUDED.period_year, period_label=EXCLUDED.period_label;
INSERT INTO curated.dim_attribute_set (attribute_hash, attributes) SELECT DISTINCT attribute_hash, attributes FROM _task218_rows ON CONFLICT (attribute_hash) DO UPDATE SET attributes=EXCLUDED.attributes;
...
def write_checksums() -> None:
    ...
    paths += sorted((RAW_DIR/"active").glob("task-218-imf-pip-*"))
    lines=[f"{sha256_file(p)}  {rel(p)}" for p in paths if p.exists() and p.is_file()]
