...
TASK_ID = "TASK-202"
CAMPAIGN_NAME = "WDI Private Sector and Business Environment Chunked Expansion Campaign"
CAMPAIGN_MODE = "Large Campaign Execution Optimization"
SOURCE_NAME = "World Bank World Development Indicators private sector, business environment, and competitiveness indicators"
COUNTRY_CATALOG_FIXTURE = PROJECT_ROOT / "data/raw/wdi_operational_phase1/wdi-phase1-all-countries-3i-2000-2023.json"
BASE_RAW_DIR = PROJECT_ROOT / "data/raw/task202_wdi_private_sector_business_environment_chunked_expansion"
BASE_PROCESSED_DIR = PROJECT_ROOT / "data/processed/task202_wdi_private_sector_business_environment_chunked_expansion"
CHECKPOINT_DIR = BASE_RAW_DIR / "checkpoints"
RAW_CHUNK_DIR = BASE_RAW_DIR / "chunks"
NORM_CHUNK_DIR = BASE_PROCESSED_DIR / "chunks"
MANIFEST_PATH = BASE_PROCESSED_DIR / "task-202-wdi-private-sector-business-environment-chunked-manifest.json"
...
def _get_json(url: str, timeout_seconds: int) -> Any:
    req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-202 WDI private sector business environment chunked expansion"})
...
def base_scope(indicators: list[str], chunk_index: int | None = None) -> dict[str, Any]:
    ...
    return {"task": TASK_ID, "campaign": CAMPAIGN_NAME, "mode": CAMPAIGN_MODE,
        ...
        "domain": "Private sector, business environment, firm formation, and competitiveness", "analytical_capability": "business environment, firm entry, credit information, infrastructure constraints, trade competitiveness, and private-sector development monitoring",
        "confidence_cell": "WDI public API v2 annual scalar country-indicator observations for private sector business environment indicators",
...
def fetch_raw(timeout_seconds: int = 45, max_workers: int = 16) -> dict[str, Any]:
    ...
    for idx, indicators in enumerate(_chunks(CANDIDATE_INDICATORS, CHUNK_SIZE), start=1):
        ...
        raw_path=RAW_CHUNK_DIR / f"task-202-wdi-private-sector-business-environment-raw-chunk-{idx:02d}.json"
...
def normalize(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for req in raw['requests']:
        ...
        for item in obs:
            ...
            rows.append({"source": SOURCE_NAME, "indicator_id": ind.get('id'), "indicator_name": ind.get('value'), "country_id": country.get('id'), "country_name": (cat.get('name') or cat.get('value') or country.get('value')), "countryiso3code": iso, "date": str(item.get('date')), "value": item.get('value'), "unit": item.get('unit') or None, "obs_status": item.get('obs_status') or None, "decimal": item.get('decimal'), "repository_section": "private_sector_business_environment_chunked", "operational_capability": "business environment, firm entry, credit information, infrastructure constraints, trade competitiveness, and private-sector development monitoring", "operational_mode": CAMPAIGN_MODE, "coverage_level": "implemented_compatible_wdi_economy_growth_chunked_annual_scalar_campaign", "region_id": (cat.get('region') or {}).get('id'), "region_label": (cat.get('region') or {}).get('value'), "income_level_id": (cat.get('incomeLevel') or {}).get('id'), "income_level_label": (cat.get('incomeLevel') or {}).get('value')})
...
def write_artifacts(raw_all: dict[str, Any]) -> dict[str, Any]:
    ...
    for idx, indicators in enumerate(_chunks(CANDIDATE_INDICATORS, CHUNK_SIZE), start=1):
        ...
        norm_path=NORM_CHUNK_DIR / f"task-202-wdi-private-sector-business-environment-normalized-chunk-{idx:02d}.json"
