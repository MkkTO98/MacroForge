...
TASK_ID = "TASK-190"
CAMPAIGN_NAME = "WDI Human Capital Foundations Repository Expansion Campaign"
...
RAW_PATH = PROJECT_ROOT / "data/raw/task190_wdi_human_capital/task-190-wdi-human-capital-45i-1990-2024.json"
NORMALIZED_PATH = PROJECT_ROOT / "data/processed/task190_wdi_human_capital/task-190-wdi-human-capital-normalized.json"
REPORT_DIR = PROJECT_ROOT / "artifacts/reports"
DATE_RANGE = "1990:2024"
PERIODS = [str(y) for y in range(1990, 2025)]
CANDIDATE_INDICATORS = [
    "SE.PRE.ENRR", "SE.PRM.NENR", "SE.SEC.NENR", "SE.PRM.TENR", "SE.SEC.TENR",
    "SE.PRM.PRSL.ZS", "SE.SEC.PROG.ZS", "SE.PRM.UNER", "SE.SEC.UNER.LO.ZS", "SE.SEC.UNER.UP.ZS",
    "SE.ENR.PRIM.FM.ZS", "SE.ENR.SECO.FM.ZS", "SE.ENR.TERT.FM.ZS", "SE.PRM.TCHR", "SE.SEC.TCHR",
    "SE.PRM.TCHR.FE.ZS", "SE.SEC.TCHR.FE.ZS", "SE.PRM.ENRL", "SE.SEC.ENRL", "SE.TER.ENRL",
    "SH.XPD.CHEX.PC.CD", "SH.XPD.CHEX.PP.CD", "SH.XPD.GHED.GD.ZS", "SH.XPD.OOPC.CH.ZS", "SH.XPD.OOPC.PC.CD",
    "SH.XPD.PVTD.CH.ZS", "SH.TBS.INCD", "SH.DYN.NMRT", "SH.DYN.MORT.FE", "SH.DYN.MORT.MA",
    "SH.IMM.HEPB", "SH.IMM.IBCG", "SH.STA.BASS.ZS", "SH.STA.SMSS.ZS", "SH.H2O.BASW.ZS",
    "SH.STA.HYGN.ZS", "SH.MLR.INCD.P3", "SH.MED.NUMW.P3", "SH.MED.CMHW.P3", "SH.DYN.AIDS.ZS",
    "HD.HCI.OVRL", "HD.HCI.OVRL.FE", "HD.HCI.OVRL.MA", "HD.HCI.EYRS", "HD.HCI.LAYS",
]
...
def fetch_raw(timeout_seconds: int = 90) -> dict[str, Any]:
    ...
    for indicator in CANDIDATE_INDICATORS:
        ...
        for _ in range(3):
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-190 WDI expansion"})
...
def normalize(raw: dict[str, Any], min_non_null_observations: int = 1) -> dict[str, Any]:
    ...
    for req in raw["requests"]:
        ...
        for item in obs:
            ...
            rows.append({"source": SOURCE_NAME, "indicator_id": ind.get("id"), "indicator_name": ind.get("value"), "country_id": country.get("id"), "country_name": (cat.get("name") or cat.get("value") or country.get("value")), "countryiso3code": iso, "date": str(item.get("date")), "value": item.get("value"), "unit": item.get("unit") or None, "obs_status": item.get("obs_status") or None, "decimal": item.get("decimal"), "repository_section": "human_capital_foundations", "operational_capability": "Human capital foundations monitoring", "operational_mode": CAMPAIGN_MODE, "coverage_level": "implemented_compatible_wdi_annual_scalar_campaign", "region_id": (cat.get("region") or {}).get("id"), "region_label": (cat.get("region") or {}).get("value"), "income_level_id": (cat.get("incomeLevel") or {}).get("id"), "income_level_label": (cat.get("incomeLevel") or {}).get("value")})
...
def reports(raw: dict[str, Any], load_counts: dict[str,int] | None=None, db_before: dict[str,int] | None=None, db_after: dict[str,int] | None=None) -> dict[str, Any]:
    ...
    return {
      "selection": {"task": TASK_ID, "status": "complete", "selected_capability": "Human capital foundations monitoring", "drdf_domain": "Human capital, education, and health foundations", "acpf_gap": "Human-capital evidence remained fragmented across a few education and health indicators, lacking broad annual country-panel coverage for enrollment flows, teachers, health spending, sanitation/water, immunization, mortality, disease burden, and human-capital index signals.", "cef_confidence_cell": "WDI public API v2 annual scalar country-indicator observations", "implementation_path": "existing WDI annual-scalar loader", "candidate_count": len(CANDIDATE_INDICATORS), "candidate_indicators": CANDIDATE_INDICATORS, "selection_rationale": "Highest-value compatible campaign because it expands investment-relevant labor-supply/productivity foundations inside the proven WDI annual-scalar architecture."},
...
def write_all(raw: dict[str, Any], load_counts: dict[str,int] | None=None, db_before: dict[str,int] | None=None, db_after: dict[str,int] | None=None) -> None:
    ...
    names={"selection":"task-190-campaign-selection-report.json","expansion":"task-190-repository-expansion-report.json","postgres_growth":"task-190-postgresql-growth-report.json","capability":"task-190-capability-improvement-report.json","exclusions":"task-190-exclusion-classification-report.json","architecture_observation":"task-190-architecture-to-reality-observation-report.json"}
