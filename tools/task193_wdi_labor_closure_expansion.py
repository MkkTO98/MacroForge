...
TASK_ID = "TASK-193"
CAMPAIGN_NAME = "WDI Global Labor Capability Closure Campaign"
CAMPAIGN_MODE = "Repository Expansion with Capability Closure Optimization"
SOURCE_NAME = "World Bank World Development Indicators / ILO modeled labor indicators"
COUNTRY_CATALOG_FIXTURE = PROJECT_ROOT / "data/raw/wdi_operational_phase1/wdi-phase1-all-countries-3i-2000-2023.json"
RAW_PATH = PROJECT_ROOT / "data/raw/task193_wdi_labor_closure/task-193-wdi-labor-closure-51i-1990-2024.json"
NORMALIZED_PATH = PROJECT_ROOT / "data/processed/task193_wdi_labor_closure/task-193-wdi-labor-closure-normalized.json"
...
CANDIDATE_INDICATORS = [
    ...
    "SL.EMP.TOTL.SP.NE.MA.ZS",
    "SL.EMP.TOTL.SP.NE.FE.ZS",
]
...
def _get_json(url: str, timeout_seconds: int) -> Any:
    req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-193 WDI labor closure expansion"})
...
def fetch_raw(timeout_seconds: int = 90) -> dict[str, Any]:
    ...
    return {
        "scope": {
            ...
            "domain": "Labor market",
            "analytical_capability": "Global labor-market status, utilization, and employment-structure monitoring",
    ...
    for req in raw["requests"]:
        ...
        for item in obs:
            ...
            rows.append({
                ...
                "operational_capability": "Global labor-market status, utilization, and employment-structure monitoring",
...
def reports(raw: dict[str, Any], load_counts: dict[str, int] | None = None, db_before: dict[str, int] | None = None, db_after: dict[str, int] | None = None) -> dict[str, Any]:
    ...
    return {
        "selection": {
            "task": TASK_ID,
            "status": "complete",
            "selected_domain": "Labor market",
            "selected_capability": "Global labor-market status, utilization, and employment-structure monitoring",
            "capability_gap": "MacroForge had broad WDI macro-financial and external-vulnerability coverage, but the labor-closure domain lacked a broad GFDD-style country-year panel for financial inclusion, institutional depth, market depth, bank efficiency, bank stability, ownership/concentration, and market structure.",
            "confidence_cell": "WDI/GFDD public API v2 annual scalar country-indicator observations",
            "candidate_count": len(CANDIDATE_INDICATORS),
            ...
            "provider_selection_note": "Provider selection followed capability selection; WDI/GFDD was used because it supplied the selected labor-closure annual scalar confidence cell.",
        },
        "expansion": {
            "task": TASK_ID,
            "status": "complete",
        ...
        "domain_progress": {
            ...
            "status": "complete",
            "domain": "Labor market",
            "capability": "Global labor-market status, utilization, and employment-structure monitoring",
            ...
            "domain_completion_assessment": "Approaching operational completeness inside the WDI/GFDD annual-scalar labor-closure confidence cell. Still not complete for detailed BOP/IIP instrument/counterparty structure, quarterly banking systems, supervisory bank-level data, full securities issuance microstructure, or non-WDI cross-provider validation.",
