...
TASK_ID = "TASK-216"
SLUG = "task216_imf_bop_phase2_campaign"
SOURCE_CODE = "IMF_SDMX_BOP_API_V1"
SOURCE_NAME = "International Monetary Fund SDMX BOP API"
SOURCE_HOME_URL = "https://data.imf.org/"
PROVIDER_DATASET_CODE = "IMF:BOP"
DATAFLOW_CODE = "BOP"
DATAFLOW_VERSION = "21.0.0"
DSD_VERSION_EXPECTED = "24.0.0"
RUN_KEY = "task-216-imf-bop-current-account-phase2"
PIPELINE_NAME = "imf_bop_current_account_phase2_campaign"
FREQUENCY = "A"
UNIT_CODE = "USD"
...
START_PERIOD = "2010"
END_PERIOD = "2024"
ACCOUNTING_ENTRY = "NETCD_T"
ACCOUNTING_ENTRY_LABEL = "Net (credits less debits)"
...
PRED_PATH = REPORT_DIR / "task-216-imf-bop-frozen-pre-execution-prediction.json"
PROVIDER_REPORT = REPORT_DIR / "task-216-imf-bop-provider-structure-and-evidence-report.json"
NORM_PATH = PROCESSED_DIR / "active" / "task-216-imf-bop-normalized.json"
MANIFEST_PATH = PROCESSED_DIR / "active" / "task-216-imf-bop-manifest.json"
LOAD_REPORT = REPORT_DIR / "task-216-imf-bop-postgresql-load-report.json"
EVAL_REPORT = REPORT_DIR / "task-216-imf-bop-prediction-evaluation.json"
EXTRACTION_REPORT = REPORT_DIR / "task-216-imf-bop-extraction-decision.json"
CHECKSUMS = REPORT_DIR / "task-216-imf-bop-artifact-checksums.txt"
SQL_PATH = REPORT_DIR / "task-216-imf-bop-load.sql"
...
def canonical_indicator_code(indicator: str) -> str:
    return f"IMF:BOP:{ACCOUNTING_ENTRY}:{indicator}:{UNIT_CODE}:SCALE_{UNIT_SCALE}:{FREQUENCY}"


def source_url(countries: list[str]) -> str:
    c = "+".join(countries)
    inds = "+".join(INDICATORS)
    return f"https://api.imf.org/external/sdmx/2.1/data/{DATAFLOW_CODE}/{c}.{ACCOUNTING_ENTRY}.{inds}.{UNIT_CODE}.{FREQUENCY}?startPeriod={START_PERIOD}&endPeriod={END_PERIOD}"


def fetch_url(url: str, *, timeout: int = 180) -> tuple[bytes, dict[str, Any]]:
    req = urllib.request.Request(url, headers={"User-Agent": "MacroForge TASK-216 IMF BOP Phase2"})
...
def fetch_metadata_active() -> dict[str, Any]:
    ...
    (attempt_dir / "task-216-imf-bop-metadata.xml").write_bytes(data)
    write_json(attempt_dir / "task-216-imf-bop-metadata.json", meta)
    active = RAW_DIR / "active"
    tmp = RAW_DIR / ".active-metadata.tmp"
    if tmp.exists():
        shutil.rmtree(tmp)
    tmp.mkdir(parents=True)
    (tmp / "task-216-imf-bop-metadata.xml").write_bytes(data)
    write_json(tmp / "task-216-imf-bop-metadata.json", meta)
    active.mkdir(parents=True, exist_ok=True)
    shutil.move(str(tmp / "task-216-imf-bop-metadata.xml"), str(active / "task-216-imf-bop-metadata.xml"))
    shutil.move(str(tmp / "task-216-imf-bop-metadata.json"), str(active / "task-216-imf-bop-metadata.json"))
    tmp.rmdir()
    return meta


def parse_metadata(metadata_path: Path | None = None) -> dict[str, Any]:
    path = metadata_path or RAW_DIR / "active" / "task-216-imf-bop-metadata.xml"
...
def write_provider_report_and_prediction() -> dict[str, Any]:
    if not (RAW_DIR / "active" / "task-216-imf-bop-metadata.xml").exists():
    ...
    for iso3 in accepted:
        for ind in INDICATORS:
            candidate_series.append({"series_key": f"{iso3}.{ACCOUNTING_ENTRY}.{ind}.{UNIT_CODE}.{FREQUENCY}", "provider_territory_code": iso3, "accounting_entry": ACCOUNTING_ENTRY, "indicator": ind, "unit": UNIT_CODE, "frequency": FREQUENCY})
    provider_report = {
        "task": TASK_ID,
        "source_api": "IMF external SDMX 2.1 API /external/sdmx/2.1/data/BOP and dataflow/all/BOP/latest?references=all",
        ...
        "institutional_sector": "encoded in selected INDICATOR where applicable; selected current-account aggregate indicators do not add sector split",
        "counterpart": "not a BOP series-key dimension for selected current-account aggregate series; provider attributes may contain source/methodology but no selected counterpart dimension",
        ...
        "selected_capability": "Annual current-account balance and component net flows by country in USD, preserving IMF BOP accounting-entry, component, unit, scale, frequency, status, and as-of evidence.",
    ...
    prediction = {
        ...
        "why_stronger_than_another_bis_or_small_proof": "The repository already has three recent BIS Phase 2 campaigns; IMF BOP closes a named external-sector gap (current account and components) and directly complements existing BOP financial-account, IIP, reserves, WDI external debt, and WDI trade evidence. This is higher marginal macroeconomic value than another BIS extension or small provider proof because current-account balances are first-order external-vulnerability inputs.",
        "canonical_source_identity": SOURCE_CODE,
        "canonical_provider_dataset_identity": PROVIDER_DATASET_CODE,
        "release_identity_rule": "derive acquired BOP as-of key from provider dataset UPDATE_DATE/PUBLICATION_DATE/Prepared evidence; do not use query-window bounds as release identity and do not invent official release semantics",
        "distinct_run_identity": RUN_KEY,
        "accepted_territories": [{"provider_code": k, "canonical_code": k, "label": v} for k, v in accepted.items()],
        "selected_accounting_components": [{"accounting_entry": ACCOUNTING_ENTRY, "accounting_entry_label": ACCOUNTING_ENTRY_LABEL, "indicator": k, "indicator_label": v} for k, v in INDICATORS.items()],
        "selected_sectors_counterparts": "Aggregate current-account components; no selected sector/counterpart split beyond component labels.",
        ...
        "expected_dimensional_complexity": "moderate: BOP has many attributes, but selected series-key dimensions are COUNTRY, BOP_ACCOUNTING_ENTRY, INDICATOR, UNIT, FREQUENCY; scalar identity is compatible if only COUNTRY is removed and all other dimensions are retained.",
        ...
        "expected_implementation_friction": "moderate-high due to existing BOP task-local constants using campaign-specific source identities and because missing/whole-series distinctions must be made explicitly",
        "expected_postgresql_growth": {"staging_rows": "observed plus explicit-missing rows only", "facts": "observed plus explicit-missing rows only", "indicators": len(INDICATORS), "dataset_releases": 1, "pipeline_runs": 1},
        "expected_scalar_compatibility": "compatible for selected aggregate current-account annual USD net-flow scalar series",
        "structural_assumptions_pressure_tested": ["source identity must be API/dataset not campaign", "release/as-of identity must come from provider metadata", "BOP component dimensions can be source-scoped scalar indicators", "missing years inside returned series are explicit missing", "whole-series absence remains distinct from missing observations"],
    }
...
def fetch_values() -> dict[str, Any]:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        try:
            ...
            meta.update({"task": TASK_ID, "status": "value_chunk_acquired", "attempt_id": attempt_id, "chunk_index": idx, "chunk_count": len(chunks), "countries": chunk, "request_parameters": {"dataflow": DATAFLOW_CODE, "countries": chunk, "accounting_entry": ACCOUNTING_ENTRY, "indicators": list(INDICATORS), "unit": UNIT_CODE, "frequency": FREQUENCY, "startPeriod": START_PERIOD, "endPeriod": END_PERIOD}})
            raw_name = f"task-216-imf-bop-values-chunk-{idx:02d}.xml"
            meta_name = f"task-216-imf-bop-values-chunk-{idx:02d}.json"
            ...
            write_json(attempt_dir / f"task-216-imf-bop-values-chunk-{idx:02d}-error.json", err)
            errors.append(err)
    if errors:
    ...
    write_json(tmp / "task-216-imf-bop-raw-values-manifest.json", manifest)
    active.mkdir(parents=True, exist_ok=True)
...
def release_key_from_responses(headers: list[dict[str, Any]]) -> str:
    ...
    if not date:
        raise ValueError("cannot derive IMF BOP as-of identity from provider metadata")
    token = date.replace(":", "").replace("-", "").replace(".", "").replace("Z", "z").replace("T", "t")[:32].lower()
    return f"imf-bop-asof-{token}"


def normalize() -> dict[str, Any]:
    pred = json.loads(PRED_PATH.read_text(encoding="utf-8"))
    active = RAW_DIR / "active"
    value_manifest = json.loads((active / "task-216-imf-bop-raw-values-manifest.json").read_text(encoding="utf-8"))
    meta = parse_metadata(active / "task-216-imf-bop-metadata.xml")
    country_labels = {x["provider_code"]: x["label"] for x in pred["accepted_territories"]}
    obs_by_series_period: dict[tuple[str, str, str], dict[str, str]] = {}
    returned_series: dict[tuple[str, str], dict[str, str]] = {}
    provider_headers = []
    incompatible = []
    for item in value_manifest["responses"]:
        ...
        for series in root.iter():
            ...
            if not c or not ind or acct != ACCOUNTING_ENTRY or ind not in INDICATORS or unit != UNIT_CODE or freq != FREQUENCY or c not in country_labels:
                incompatible.append(attrs)
                continue
            returned_series[(c, ind)] = attrs
            for obs in series:
                if local_name(obs.tag) == "Obs" and obs.attrib.get("TIME_PERIOD"):
                    obs_by_series_period[(c, ind, obs.attrib["TIME_PERIOD"])] = dict(obs.attrib)
    p = periods()
    rows = []
    explicit_missing = 0
    observed = 0
    ...
    for c in country_labels:
        for ind, label in INDICATORS.items():
            if (c, ind) not in returned_series:
                whole_series_absence.append({"provider_territory_code": c, "indicator": ind, "category": "whole_series_absence"})
                continue
            sattrs = returned_series[(c, ind)]
            for per in p:
                obs = obs_by_series_period.get((c, ind, per))
                status = "observed"
                value = None
                ...
                attrs = {
                    ...
                    "bop_accounting_entry_label": ACCOUNTING_ENTRY_LABEL,
                    "bop_indicator": ind,
                    "bop_indicator_label": label,
                    "unit": UNIT_CODE,
                    ...
                    "value_status": "provider_bop_value_status_unspecified",
                }
                ah = hashlib.sha256(json.dumps(attrs, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
                rows.append({"provider_indicator_code": canonical_indicator_code(ind), "provider_indicator_label": label, "territory_code": c, "territory_label": country_labels[c], "provider_period_code": per, "period_year": int(per), "value": value, "unit_code": f"{UNIT_CODE}_SCALE_{sattrs.get('SCALE', UNIT_SCALE)}", "unit_label": f"{UNIT_LABEL}, millions", "observation_status": status, "decimal_precision": None if obs is None else obs.get("PRECISION"), "attribute_hash": ah, "attributes": attrs, "source_payload": {"series_attributes": sattrs, "observation_attributes": obs or {}, "source_url_count": len(value_manifest["responses"])}})
...
def build_load_sql(norm: dict[str, Any], run_key: str = RUN_KEY) -> str:
    ...
    raw_paths = [rel(p) for p in sorted((RAW_DIR / "active").glob("task-216-imf-bop-values-chunk-*.xml"))]
...
VALUES ({sql_literal(SOURCE_CODE)}, {sql_literal(SOURCE_NAME)}, {sql_literal(SOURCE_HOME_URL)}, 'IMF public SDMX BOP API')
...
SELECT {sql_literal(run_key)}, s.source_id, r.dataset_release_id, {sql_literal(PIPELINE_NAME)}, now(), 'succeeded', {jsonb_literal({'task': TASK_ID, 'startPeriod': START_PERIOD, 'endPeriod': END_PERIOD, 'indicators': list(INDICATORS), 'accounting_entry': ACCOUNTING_ENTRY})}, {jsonb_literal({'normalized_path': rel(NORM_PATH), 'manifest_path': rel(MANIFEST_PATH), 'raw_paths': raw_paths})} FROM _source_row s CROSS JOIN _release_row r
...
SELECT DISTINCT s.source_id, indicator_code, indicator_name, 'IMF BOP current-account component source-scoped scalar indicator preserving accounting entry, unit, scale, and frequency', 'External Sector / Balance of Payments' FROM _task216_rows CROSS JOIN _source_row s
...
INSERT INTO curated.dim_unit (unit_code, unit_name, unit_description) SELECT DISTINCT unit_code, unit_label, 'IMF BOP unit with provider scale preserved' FROM _task216_rows ON CONFLICT (unit_code) DO UPDATE SET unit_name=EXCLUDED.unit_name, unit_description=EXCLUDED.unit_description;
INSERT INTO curated.dim_period (frequency, period_year, period_start_date, period_end_date, period_label)
SELECT DISTINCT 'A', period_year, make_date(period_year,1,1), make_date(period_year,12,31), provider_period_code FROM _task216_rows
...
SELECT DISTINCT attribute_hash, attributes FROM _task216_rows ON CONFLICT (attribute_hash) DO UPDATE SET attributes=EXCLUDED.attributes;
...
FROM _task216_rows r CROSS JOIN _source_row s CROSS JOIN _release_row rel CROSS JOIN _run_row pr
...
INSERT INTO meta.lineage_event (pipeline_run_id, source_id, event_type, from_artifact, to_artifact, checksum_sha256, row_count, details) SELECT r.pipeline_run_id, s.source_id, 'task216_imf_bop_current_account_loaded', {sql_literal(rel(RAW_DIR / 'active'))}, {sql_literal(rel(NORM_PATH))}, {sql_literal(sha256_file(NORM_PATH))}, (SELECT count(*) FROM _task216_rows), {jsonb_literal(metadata)} FROM _run_row r CROSS JOIN _source_row s;
INSERT INTO meta.quality_check (pipeline_run_id, check_name, check_status, observed_value, expected_value, details)
SELECT pipeline_run_id, 'task216_candidate_reconciliation', CASE WHEN (SELECT count(*) FROM _task216_rows)={len(norm['rows'])} THEN 'pass' ELSE 'fail' END, (SELECT count(*) FROM _task216_rows), {len(norm['rows'])}, {jsonb_literal(metadata)} FROM _run_row
UNION ALL SELECT pipeline_run_id, 'task216_observed_missing_reconciliation', CASE WHEN (SELECT count(*) FILTER (WHERE observation_status='observed') FROM _task216_rows)={norm['observed_value_count']} AND (SELECT count(*) FILTER (WHERE observation_status='missing') FROM _task216_rows)={norm['explicit_missing_value_count']} THEN 'pass' ELSE 'fail' END, (SELECT count(*) FROM _task216_rows), {len(norm['rows'])}, {jsonb_literal(metadata)} FROM _run_row;
COMMIT;
"""
...
SELECT (SELECT count(*) FROM staging.task216_imf_bop_current_account_observation st JOIN run r ON st.pipeline_run_id=r.pipeline_run_id)::text || '|' ||
...
def evaluate_prediction() -> dict[str, Any]:
    ...
    report = {"task": TASK_ID, "prediction_verdict": verdict, "expected_candidate_cells": pred["expected_candidate_cells"], "actual_candidate_cells": norm["candidate_cell_count"], "expected_provider_valued_facts": pred["expected_provider_valued_facts"], "actual_provider_valued_facts": norm["observed_value_count"], "scale_prediction_error": 0, "missingness_prediction_error": missing_error, "provider_behavior_surprises": "Country chunking worked; provider returned whole-series absences for some country/component pairs rather than explicit null observations.", "territory_or_unit_surprises": "No unit incompatibility for selected USD annual current-account series; unsupported provider territories remained outside canonical country catalogue.", "implementation_friction_error": "High but expected: release/as-of and whole-series absence separation required task-local code; no schema contradiction.", "architecture_compatibility": "compatible"}
    write_json(EVAL_REPORT, report)
    return report


def write_extraction_decision() -> dict[str, Any]:
    report = {"task": TASK_ID, "decision": "No IMF-BOP-specific shared substrate extracted", "evidence_compared": ["TASK-063 bounded BOP financial-account proof", "TASK-136 G7 BOP financial-account deepening", "TASK-158 BOP operational evolution", "TASK-216 current-account Phase 2"], "repeated_responsibilities": ["IMF BOP SDMX metadata parsing", "series-key dimension preservation", "BOP accounting-entry and indicator labels", "source/dataset/release identity discipline"], "rejected_extraction_reason": "Duplication is visible, but TASK-216 corrected identity semantics and broadened component family without stable enough repeated implementation surface to justify an IMF-BOP substrate now. The earlier bounded implementations used campaign-specific source identities; extracting before reconciling those would freeze drift. Keep source-specific task-local code and revisit after one more BOP/IIP relationship campaign.", "explicitly_not_created": ["generic IMF framework", "universal SDMX adapter", "generic accounting ontology", "universal campaign engine", "multidimensional schema"]}
...
def write_checksums() -> None:
    ...
    paths += sorted((RAW_DIR / "active").glob("task-216-imf-bop-*"))
    lines = []
    for path in paths:
