...
## Active Work
- `TASK-PF-20260801-ignored-artifact-preservation-governance.md` preserves every earlier block, correction, and audit through the latest `BLOCKED / PUBLICATION TRANSITION DEFECT`. The sixth correction minimally extends the existing lifecycle mechanism with a fail-closed state-conditioned transition evaluator; live authority is derived from authenticated Git plus exact-byte external review evidence rather than asserted as a mutable pending/sole-next-gate status.
